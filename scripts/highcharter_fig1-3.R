...
for (industry_data in hc_data) {
  ...
  data_points_list <- lapply(1:nrow(data_points), function(i) {
    point <- data_points[i, ]
    marker <- if (point$x %in% c(1990, 1995, 2023)) {
  ...
  # Add the label for 2023 point
  last_point_2023 <- data_points_list[[which(data_points$x == 2023)]]
  last_point_2023$dataLabels <- list(
    enabled = TRUE,
    align = 'left',
  ...
  data_points_list[[which(data_points$x == 2023)]] <- last_point_2023
...
# Filter data for the year 2023 and exclude 'Total'
data_2023 <- subsector_total %>%
  filter(Year == 2023 & Subsector != 'Total') %>%
  arrange(desc(Value)) %>%
...
color_map <- setNames(preset_colors[1:nrow(data_2023)], data_2023$Subsector)

# Generate data list for highcharter
data_list <- lapply(1:nrow(data_2023), function(i) {
  list(
    name = data_2023$Subsector[i],
    y = data_2023$Value[i],
    color = color_map[[data_2023$Subsector[i]]]
  )
})
...
fig2 <- highchart() %>%
  ...
  hc_xAxis(categories = data_2023$Subsector, 
           labels = list(style = list(color = "#000000", fontSize = '16px', fontFamily = 'Arial'))) %>%
  hc_yAxis(title = list(text = "Emissions (MtCO₂e)", style = list(color = "#000000",fontSize = '16px', fontFamily = 'Arial')),
           labels = list(style = list(color = "#000000", fontSize = '16px', fontFamily = 'Arial')),
           tickInterval = 0.5) %>%
...
# arrange in descending order- first calc total source emissions across all subsectors
#order will be accrodign to "source" column

sub_total <- subsector_source %>% mutate(total = rowSums(.[, -1])) %>% arrange(total)
# Invert
# data_long <- data_long %>%
#   arrange(match(Source, desc(unique(Source))))

data_long <- data_long %>% arrange(match(Source, sub_total$Source))

sub_order <- c("Suckler beef", "Arable", "Dairy", "Sheep", "Other", "Dairy beef")
...
series_data <- lapply(unique_sources, function(source) {
  list(
    ...
    data = data_long %>%
      ...
      arrange(match(Subsector,sub_order)) %>%
      pull(Value),
